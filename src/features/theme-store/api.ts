...
import {
  THEME_STORE_API_BASE,
  THEME_STORE_TOKEN_KEY,
} from "../../shared/config/brand";

const API_BASE = THEME_STORE_API_BASE;

function requireApiBase(): string {
  if (!API_BASE) {
    throw new Error("Theme store API is not configured");
  }
  return API_BASE;
}

function getToken(): string | null {
  return localStorage.getItem(THEME_STORE_TOKEN_KEY);
}
