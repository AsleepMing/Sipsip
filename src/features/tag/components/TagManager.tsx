...
export default function TagManager({ t, theme }: TagManagerProps) {
    ...
    const [sidebarWidth, setSidebarWidth] = useState(() => {
        try {
            const saved = Number(window.localStorage.getItem(TAG_MANAGER_SIDEBAR_WIDTH_KEY));
            return Number.isFinite(saved) ? Math.min(Math.max(saved, 110), 320) : 160;
        } catch {
            return 160;
        }
    });
    const [sidebarHeight, setSidebarHeight] = useState(() => {
        try {
            const saved = Number(window.localStorage.getItem(TAG_MANAGER_SIDEBAR_HEIGHT_KEY));
            return Number.isFinite(saved) ? Math.min(Math.max(saved, 120), 420) : 180;
        } catch {
            return 180;
        }
    });
