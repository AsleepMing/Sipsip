...
const readCachedString = (key: string, fallback = "") => {
  try {
...
const readCachedNumber = (key: string, fallback: number) => {
  const raw = readCachedString(key, "");
...
export const useAppState = (): AppState => {
  ...
  const [customBackground, setCustomBackground] = useState<string>(() =>
    readCachedString("tiez_custom_background")
  );
  const [customBackgroundOpacity, setCustomBackgroundOpacity] = useState(() =>
    readCachedNumber("tiez_custom_background_opacity", 45)
  );
  const [surfaceOpacity, setSurfaceOpacity] = useState(() =>
    readCachedNumber("tiez_surface_opacity", 50)
  );
  ...
  const [cloudSyncWebdavBasePath, setCloudSyncWebdavBasePath] = useState("sipsip-sync");
