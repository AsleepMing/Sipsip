...
export const useSettingsPostInit = ({
  ...
  useEffect(() => {
    ...
    if (settings["app.custom_background"] !== undefined) {
      setCustomBackground(settings["app.custom_background"]);
    }
    if (settings["app.custom_background_opacity"] !== undefined) {
      const next = parseInt(settings["app.custom_background_opacity"]);
      if (Number.isFinite(next)) {
        setCustomBackgroundOpacity(Math.min(100, Math.max(0, next)));
      }
    }
    if (settings["app.surface_opacity"] !== undefined) {
    ...
    setCloudSyncWebdavBasePath(settings["cloud_sync_webdav_base_path"] || "sipsip-sync");
