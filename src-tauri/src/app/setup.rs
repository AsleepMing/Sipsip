...
pub fn init(app: &mut App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    allow_data_assets(app, &app_dir);

    // 2. Logger Initialization
    crate::logger::init(app_dir.join("sipsip.log"));
    info!(">>> [STARTUP] Sipsip starting up...");
    ...
    let conn = database::init_db(&db_path_str).map_err(|e| {
        ...
        WindowExt::show_error_box("Sipsip 启动错误", &err_msg);
        e
    })?;
...
fn allow_data_assets(app: &App, app_dir: &std::path::Path) {
    let asset_scope = app.asset_protocol_scope();
    let _ = asset_scope.allow_directory(app_dir.join("custom_background"), true);
    let _ = asset_scope.allow_directory(app_dir.join("attachments"), true);
    let _ = asset_scope.allow_directory(app_dir.join("emoji_favorites"), true);
}
        ...
        if let Ok(entries) = std::fs::read_dir(&temp_dir) {
            for entry in entries.flatten() {
                if let Ok(name) = entry.file_name().into_string() {
                    if name.starts_with("TieZ_Clip_") || name.starts_with("Sipsip_Clip_") {
...
fn init_announcement_ping(app: &App, repo: &impl SettingsRepository) {
    ...
    if repo.get("app.last_ping_date").unwrap_or(None).as_deref() != Some(&today) {
        ...
        if let Ok(base_url) = std::env::var("SIPSIP_ANNOUNCEMENT_PING_URL")
            .or_else(|_| std::env::var("TIEZ_ANNOUNCEMENT_PING_URL"))
        {
...
fn setup_tray(app: &App, hide_tray: bool) {
    ...
    let quit_i = MenuItem::with_id(app, "quit", "退出 Sipsip", true, None::<&str>).unwrap();
