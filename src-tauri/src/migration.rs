...
const OLD_APP_DIR_NAMES: &[&str] = &["贴汁", "TieZ", "com.tiez"];
const OLD_LOG_NAMES: &[&str] = &["tiez.log", "sipsip.log"];

/// Brand migration: 贴汁 / TieZ / com.tiez -> Sipsip / com.sipsip.app
pub fn perform_migration_v028(default_app_dir: &PathBuf) {
    ...
    if let Some(parent) = default_app_dir.parent() {
        for dir_name in OLD_APP_DIR_NAMES {
            old_app_dirs_to_check.push(parent.join(dir_name));
        }
    }
    ...
    if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
        for dir_name in OLD_APP_DIR_NAMES {
            old_app_dirs_to_check.push(std::path::PathBuf::from(&local_app_data).join(dir_name));
        }
    }
    ...
    if let Ok(roaming_app_data) = std::env::var("APPDATA") {
        for dir_name in OLD_APP_DIR_NAMES {
            old_app_dirs_to_check.push(std::path::PathBuf::from(&roaming_app_data).join(dir_name));
        }
    }
    ...
    for old_app_dir in old_app_dirs_to_check {
        if old_app_dir.exists() && old_app_dir.is_dir() {
            ...
            if !default_app_dir.exists() && !success {
                println!(">>> [MIGRATION] Renaming old data folder to 'Sipsip'...");
                success = std::fs::rename(&old_app_dir, &default_app_dir).is_ok();
            } else if old_db.exists() && !new_db.exists() {
                println!(">>> [MIGRATION] Pulling old data into 'Sipsip'...");
                let _ = std::fs::create_dir_all(&default_app_dir);
                if std::fs::copy(&old_db, &new_db).is_ok() {
                    success = true;
                    for log_name in OLD_LOG_NAMES {
                        let old_log = old_app_dir.join(log_name);
                        if old_log.exists() {
                            let _ = std::fs::copy(&old_log, default_app_dir.join("sipsip.log"));
                            break;
                        }
                    }
                }
                ...
                if old_size > new_size && new_size < 50_000 {
                    ...
                    if std::fs::copy(&old_db, &new_db).is_ok() {
                        success = true;
                        println!(">>> [MIGRATION] Successfully migrated old database to Sipsip.");
                        let old_redirect = old_app_dir.join("datapath.txt");
                        if old_redirect.exists() {
                            let _ =
            ...
            if success {
                println!(">>> [CLEANUP] Cleaning up residues of old versions...");
...
/// Brand migration: Registry cleanup - returns found install location if any
        ...
        if let Ok(key) = hkcu.open_subkey_with_flags(path, KEY_READ | KEY_WRITE) {
            for subkey_name in key.enum_keys().filter_map(|x| x.ok()) {
                if let Ok(subkey) = key.open_subkey(&subkey_name) {
                    ...
                    if name.contains("贴汁") || name.contains("TieZ") {
                        println!(
...
/// Brand migration: Start Menu & Desktop cleanup
pub fn cleanup_old_start_menu() {
    #[cfg(windows)]
    {
        if let Ok(app_data) = std::env::var("APPDATA") {
            ...
            // Delete old shortcut
            for shortcut_name in ["贴汁.lnk", "TieZ.lnk"] {
                let old_lnk = start_menu.join(shortcut_name);
                if old_lnk.exists() {
                    println!(
            ...
            for folder_name in ["贴汁", "TieZ"] {
                let old_folder = start_menu.join(folder_name);
        ...
        if let Ok(user_profile) = std::env::var("USERPROFILE") {
            ...
            for shortcut_name in ["贴汁.lnk", "TieZ.lnk"] {
                let old_desktop_lnk = desktop.join(shortcut_name);
                println!(
...
/// Brand migration: clean up old installation directories
pub fn cleanup_old_install_folder(custom_path: Option<PathBuf>) {
    #[cfg(windows)]
    {
        ...
        let mut possible_paths = vec![
            ...
            std::env::var("LOCALAPPDATA")
                .ok()
                .map(|p| PathBuf::from(p).join("Programs").join("TieZ")),
            std::env::var("ProgramFiles")
                .ok()
                .map(|p| PathBuf::from(p).join("贴汁")),
            std::env::var("ProgramFiles")
                .ok()
                .map(|p| PathBuf::from(p).join("TieZ")),
