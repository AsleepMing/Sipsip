...
<body class="theme-{theme} {mode_class}">
    ...
    <script>
        ...
        const deviceId = localStorage.getItem('sipsip_device_id') || ('m-' + Math.random().toString(36).substr(2, 9));
        localStorage.setItem('sipsip_device_id', deviceId);
        
        const deviceName = "Mobile";
        const SIPSIP_LOGO = "{logo_base64}";
        ...
        function createMessageElement(direction, content, senderName, msgType, file_path) {{
            ...
            div.innerHTML = `
                ${{direction === 'received' ? (() => {{
                    ...
                    const isPc = name === '电脑' || name === 'PC' || lower === 'pc' || lower === 'sipsip';
                    if (isPc) {{
                        return `<div class="avatar"><img src="${{SIPSIP_LOGO}}" alt="Sipsip"></div>`;
                    }}
