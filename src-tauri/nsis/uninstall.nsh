...
!macro NSIS_HOOK_PREUNINSTALL
  DetailPrint "Stopping Sipsip before uninstall..."

  # 应用关闭主窗口时会缩到托盘，卸载器不能依赖普通的关闭请求。
  # 这里在开始删除文件前强制结束已安装的进程，避免 exe 被占用。
  nsExec::ExecToLog '"$SYSDIR\taskkill.exe" /F /T /IM Sipsip.exe'
  nsExec::ExecToLog '"$SYSDIR\taskkill.exe" /F /T /IM sipsip.exe'
  ...
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Sipsip"
...
!macro NSIS_HOOK_POSTUNINSTALL
  ...
  DeleteRegKey HKCU "Software\sipsip\Sipsip"
  DeleteRegKey /ifempty HKCU "Software\sipsip"
  ...
  nsExec::Exec '"$SYSDIR\cmd.exe" /C start "" /MIN powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "Start-Sleep -Seconds 3; Stop-Process -Name Sipsip,sipsip,TieZ,tiez-app -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath ''$INSTDIR'' -Force -Recurse -ErrorAction SilentlyContinue"'
!macroend
